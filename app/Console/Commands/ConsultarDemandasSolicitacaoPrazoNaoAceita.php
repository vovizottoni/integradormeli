...
class ConsultarDemandasSolicitacaoPrazoNaoAceita extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'consultardemandassolicitacaoprazonaoaceita';
     ...
     * @var string
     */
    protected $description = 'Consulta demandas com solicitação de prazo não aceita no BACEN WebService na data corrente e armazena na BASE MYSQL do Robô, depois envia para o Zendesk';
    ...
    public function handle()
    {
        ...
        try{
            ...
            $soapClient = new \SoapClient($URL_WEBSERVICE_RDR_BACEN, $optionsSoap);              
                ...
                LogsErros::create(['comando' => 'consultardemandassolicitacaoprazonaoaceita', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN indisponível ou credenciais desativadas/alteradas', 'codigo' => 1 ]);
                ...
                Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value){
                    $message->to($to_email)
                      ...
                      ->setBody('<b>comando:</b> consultardemandassolicitacaoprazonaoaceita<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN indisponível ou credenciais desativadas/alteradas'.'<br><br><b>Código de erro:</b> 1'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
                });     
        ...
        //requisita demandas com solicitação de prazo nao aceita
        //Algoritmo
        
        //Total de demandas "solicitação de prazo nao aceita" na data corrente
        $parametros = [
            ...
            'tipoConsulta' => "S", //Respondidas na data      
            'situacao' => 135  //Pendente: IF/AC Solicitação de prazo não aceita                   
        ];
        ...
        try{
            ...
            LogsErros::create(['comando' => 'consultardemandassolicitacaoprazonaoaceita', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN indisponível ou credenciais desativadas/alteradas ou método getTotalDemandas indisponível', 'codigo' => 2 ]);
            ...
            Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value){
                $message->to($to_email)
                  ->subject('Log - integrador')
                  ->from('joao.peixoto.cd@gmail.com')  
                  ->setBody('<b>comando:</b> consultardemandassolicitacaoprazonaoaceita<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN indisponível ou credenciais desativadas/alteradas ou método getTotalDemandas indisponível'.'<br><br><b>Código de erro:</b> 2'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
            });     
        ...
        if(!is_object($totalDemandas) ||  (is_object($totalDemandas) && !property_exists($totalDemandas, "return")) || (is_object($totalDemandas) && property_exists($totalDemandas, "return") && !is_int($totalDemandas->return))){
            ...
            LogsErros::create(['comando' => 'consultardemandassolicitacaoprazonaoaceita', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN retornou um valor inválido para o número de demandas solicitacao de prazo nao aceita que foi consultado', 'codigo' => 3 ]);  
            ...
            Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value){
                $message->to($to_email)
                  ->subject('Log - integrador')
                  ->from('joao.peixoto.cd@gmail.com')  
                  ->setBody('<b>comando:</b> consultardemandassolicitacaoprazonaoaceita<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN retornou um valor inválido para o número de demandas solicitacao de prazo nao aceita que foi consultado'.'<br><br><b>Código de erro:</b> 3'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
            });     
        ...
        if($totalDemandas->return == 0){ //Nenhuma nova demanda procedente
            ...
            LogsErros::create(['comando' => 'consultardemandassolicitacaoprazonaoaceita', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Ocorrência: nenhuma  demanda solicitacao de prazo nao aceita foi encontrada', 'codigo' => 4 ]);   
        ...
        for($i = 0 ; $i < $npaginas ; ++$i){
            ...
            $parametros = [
                ...
                'dataFim' => $this->getDataCorrente().'T23:59:00.000', //Alterar para data corrente do servidor  *************!!!!!!!      
                'tipoConsulta' => "S", //Respondidas na data      
                'situacao' => 135,  //Pendente: IF/AC Solicitação de prazo nao aceita                   
                'pagina' => $i
            ];
                    ...
                    foreach($demandas->return as $key_demanda => $demanda){                     
                        if(is_object($demanda)){
                            ...
                            if(!is_object($demanda_detalhes) || (is_object($demanda_detalhes) && !property_exists($demanda_detalhes, "return")) || !is_int($demanda_detalhes->return->idInterno) || !is_int($demanda_detalhes->return->idEncaminhamento)){
                                   ...
                                   LogsErros::create(['comando' => 'consultardemandassolicitacaoprazonaoaceita', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Falha na leitura de demanda do WebServive BACEN: Demanda com valores obrigatórios ausentes', 'codigo' => 5 ]);  
                            ...
                            if(empty($checka_DemandaEXISTE)){
                                ...
                                LogsErros::create(['comando' => 'consultardemandassolicitacaoprazonaoaceita', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Demanda foi teve solicitacao de prazo nao aceita mas não existe no zendesk nem robô meli. Portanto fica presa no robô e não será enviada ao zendesk.', 'codigo' => 70, 'demandas_id' => $demanda_detalhes->return->idInterno ]);    
                                ...
                                Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $n_demanda_p_email){
                                    ...
                                    ->subject('Log - integrador')
                                    ->from('joao.peixoto.cd@gmail.com')  
                                    ->setBody('<b>comando:</b> consultardemandassolicitacaoprazonaoaceita<br><br>'.'<b>Mensagem:</b> Demanda teve solicitacao de prazo nao aceita mas não existe no zendesk nem robô meli. Portanto fica presa no robô e não será enviada ao zendesk.'.'<br><br><b>Código de erro:</b> 70'.'<br><b>Instituição:</b>'.$instituicao_corrente_value.' <br><b>Numero Demanda:</b>'.$n_demanda_p_email, 'text/html');
                                });        
                                ...
                                //A demanda existente não pode ter os status:  (evitar processamento repetido) 
                                    //'situacao_no_robo' => 'solicitacao_prazo_aceita_nao_enviada_ao_zendesk'
                                    //'situacao_no_robo' => 'solicitacao_prazo_aceita_enviada_ao_zendesk'                                
                                if($checka_DemandaEXISTE->situacao_no_robo == 'solicitacao_prazo_nao_aceita_nao_enviada_ao_zendesk' || $checka_DemandaEXISTE->situacao_no_robo == 'solicitacao_prazo_nao_aceita_enviada_ao_zendesk'){
                            ...
                            if($situacao_bacen_id){
                                ...
                                Demandas::where([['id', '=', $checka_DemandaEXISTE->id]])->update(['situacao_bacen_id' => $situacao_bacen_id, 'situacao_no_robo' => 'solicitacao_prazo_nao_aceita_nao_enviada_ao_zendesk' ]);
                            ...
                            //Atualiza Ticket no Zendesk, criando um comentário sobre o novo status (Solicitação de prazo aceita) e atualizando o campo FIELD_STATUS_FINAL_DO_CASO                                                 
                            try{ 
                                
                                $body = new \StdClass();
                                $body->ticket = new \StdClass();                                                                                                
                                $body->ticket->comment = new \StdClass();
                                $body->ticket->comment->body = "Demanda com Solicitação de Prazo Não Aceita";
                                ...
                                $obj1->value = 'pendente_if_ac_solicitacao_de_prazo_nao_aceita';
                                ...
                                LogsErros::create(['comando' => 'consultardemandassolicitacaoprazonaoaceita', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Não foi possivel conectar-se ao zendesk para criar o comentário do ticket (comentário: Solicitação de prazo não aceita) :'.$e->getMessage() , 'codigo' => 71, 'demandas_id' => $checka_DemandaEXISTE->id ]);            
                                ...
                                Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $n_demanda_p_email){
                                    ...
                                    ->subject('Log - integrador')
                                    ->from('joao.peixoto.cd@gmail.com')  
                                    ->setBody('<b>comando:</b> consultardemandassolicitacaoprazonaoaceita<br><br>'.'<b>Mensagem:</b> Demanda foi solicitação de prazo não aceita mas não foi possivel conectar-se ao zendesk para criar o comentário do ticket (comentário: Solicitação de prazo não aceita). Falha ao enviar ao Zendesk.'.'<br><br><b>Código de erro:</b> 71'.'<br><b>Instituição:</b>'.$instituicao_corrente_value.' <br><b>Numero Demanda:</b>'.$n_demanda_p_email, 'text/html');
                                });     
                            ...
                            Demandas::where([['id', '=', $checka_DemandaEXISTE->id]])->update(['situacao_no_robo' => 'solicitacao_prazo_nao_aceita_enviada_ao_zendesk' ]);       
       ...
       //Display Script End time
        ...
        if($totalDemandas->return && is_int($totalDemandas->return)){
            LogsComandos::create(['comando' => 'consultardemandassolicitacaoprazonaoaceita', 'data' => date('Y-m-d H:i:s'), 'duracao' => $execution_time, 'totalDemandasRetornadasRDRWebservice' => $totalDemandas->return ]);
        }              
