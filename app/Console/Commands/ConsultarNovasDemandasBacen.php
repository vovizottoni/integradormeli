...
class ConsultarNovasDemandasBacen extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'consultarnovasdemandasbacen';
     ...
     * @var string
     */
    protected $description = 'Consulta novas demandas no BACEN WebService na data corrente e armazena na BASE MYSQL do Robô, depois envia para o Zendesk';
    ...
    public function handle()
    {
        ...
        try{
            ...
            $soapClient = new \SoapClient($URL_WEBSERVICE_RDR_BACEN, $optionsSoap);              
                ...
                LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN indisponível ou credenciais desativadas/alteradas', 'codigo' => 1 ]);
                ...
                Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value){
                    $message->to($to_email)
                      ...
                      ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN indisponível ou credenciais desativadas/alteradas'.'<br><br><b>Código de erro:</b> 1'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
                });     
        ...
        try{
            ...
            LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN indisponível ou credenciais desativadas/alteradas ou método getTotalDemandas indisponível', 'codigo' => 2 ]);
            ...
            Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value ){
                $message->to($to_email)
                  ->subject('Log - integrador')
                  ->from('joao.peixoto.cd@gmail.com')  
                  ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN indisponível ou credenciais desativadas/alteradas ou método getTotalDemandas indisponível'.'<br><br><b>Código de erro:</b> 2'.'<br><b>Instituição:</b>'.$instituicao_corrente_value , 'text/html');
            });     
        ...
        if(!is_object($totalDemandas) ||  (is_object($totalDemandas) && !property_exists($totalDemandas, "return")) || (is_object($totalDemandas) && property_exists($totalDemandas, "return") && !is_int($totalDemandas->return))){
            ...
            LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN retornou um valor inválido para o número de novas demandas que foi consultado', 'codigo' => 3 ]);  
            ...
            Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value ){
                $message->to($to_email)
                  ->subject('Log - integrador')
                  ->from('joao.peixoto.cd@gmail.com')  
                  ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN retornou um valor inválido para o número de novas demandas que foi consultado'.'<br><br><b>Código de erro:</b> 3'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
            });     
        ...
        if($totalDemandas->return == 0){ //Nenhuma nova demanda
            ...
            LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Ocorrência: nenhuma nova demanda foi encontrada na Instituição corrente'.' Instituição:'.$value_Inst['user'] , 'codigo' => 4 ]);   
                        ...
                        if(is_object($demanda)){
                            ...
                            if(!is_object($demanda_detalhes) || (is_object($demanda_detalhes) && !property_exists($demanda_detalhes, "return")) || (is_object($demanda_detalhes) && property_exists($demanda_detalhes, "return") && !property_exists($demanda_detalhes->return, "instituicao")) || !is_object($demanda_detalhes->return->instituicao) || !is_int($demanda_detalhes->return->instituicao->idBacen) ||
                               ...
                               !is_int($demanda_detalhes->return->idEncaminhamento)){
                                   ...
                                   LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Falha na leitura de demanda do WebServive BACEN: Demanda com valores obrigatórios ausentes', 'codigo' => 5 ]);  
                            ...
                            //checka se é uma demanda repetida
                            ...
                            $demanda_inserir = [
                                ...
                                'situacao_no_robo' => 'nova_nao_enviada_ao_zendesk' //primeira situação de uma demanda no fluxo
                            ];
                            $demanda_inserida = Demandas::create($demanda_inserir);
                            $demanda_inserida_id = $demanda_inserida->id;

                            //insere cidadao
                            if(property_exists($demanda_detalhes->return, 'cidadao') && is_object($demanda_detalhes->return->cidadao) && !empty($demanda_detalhes->return->cidadao->nome) && !empty($demanda_detalhes->return->cidadao->emails) && filter_var($demanda_detalhes->return->cidadao->emails, FILTER_VALIDATE_EMAIL)){ 
                                ...
                                //Utiliza o usuário fictício 
                                //Utiliza o usuário fictício
                                //Utiliza o usuário fictício

                                $demanda_detalhes->return->cidadao->nome = "Cidadão Fictício";
                                $demanda_detalhes->return->cidadao->documento = "99999999";
                                $demanda_detalhes->return->cidadao->tipoDocumento = "tipo1";
                                $demanda_detalhes->return->cidadao->emails = "bacen_cidadao_ficticio@teste.com";
                                $demanda_detalhes->return->cidadao->telefones = "99999999";
                                $demanda_detalhes->return->cidadao->cep = "99999999";
                                $demanda_detalhes->return->cidadao->uf = "ficticio";
                                $demanda_detalhes->return->cidadao->municipio->descricao = "FICTICIO";
                                $demanda_detalhes->return->cidadao->municipio->id = "9999";
                                $demanda_detalhes->return->cidadao->endereco = "ficticio";
                                $demanda_detalhes->return->cidadao->bairro = "ficticio";
                                $demanda_detalhes->return->cidadao->numero = "9999";
                                $demanda_detalhes->return->cidadao->complemento = "ficticio";
                                $demanda_detalhes->return->cidadao->cidadeUF = "ficticio";
                                ...
                                //Concatena ao comentário Zendesk informando que o usuário deste ticket é fictício
                                //Concatena ao comentário Zendesk informando que o usuário deste ticket é fictício
                                ...
                                if($demanda_detalhes->return->descricao){
                                    $demanda_detalhes->return->descricao .= "<br><br><b>Atenção</b>, a demanda foi cadastrada no Bacen sem um e-mail, usamos um e-mail genérico (bacen_cidadao_ficticio@teste.com) para criação do caso. Por favor atualize o email neste ticket, antes de entrar em contato com o cliente";
                                }else{
                                    $demanda_detalhes->return->descricao = "<br><br><b>Atenção</b>, a demanda foi cadastrada no Bacen sem um e-mail, usamos um e-mail genérico (bacen_cidadao_ficticio@teste.com) para criação do caso. Por favor atualize o email neste ticket, antes de entrar em contato com o cliente";
                                }         
                                       ...
                                       if(is_object($anexo_it) && $anexo_it->url && $anexo_it->nome){
                                           //captura anexo com requisição autenticada
                                            try{ 
                                                ...
                                                LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Não foi possivel capturar do RDR-BACEN o anexo: '.$anexo_it->url.' .Demanda não enviada ao zendesk. erro:'.$e->getMessage(), 'codigo' => 88, 'demandas_id' => $demanda_inserida_id ]);                                                           
            
                                                //envia email
                                                $anexo_it_corrente_value = $anexo_it->url;
                                                $numeroDemanda_corrente_value = $demanda_detalhes->return->numeroDemanda;
                                                Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $numeroDemanda_corrente_value, $anexo_it_corrente_value){
                                                    $message->to($to_email)
                                                    ->subject('Log - integrador')
                                                    ->from('joao.peixoto.cd@gmail.com')  
                                                    ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b> Não foi possivel capturar do RDR-BACEN o anexo: '.$anexo_it_corrente_value.'<br> Número Demanda: '.$numeroDemanda_corrente_value.'<br><br><b>Código de erro:</b> 88'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
                                                });     

                                                //Não envia ao zendesk e pula para o próximo anexo. 
                                                continue;
                                            }  
                            ...
                            //ENVIA DADOS PARA ZENDESK 
                                ...
                                try{ 
                                    ...
                                    LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Não foi possivel conectar-se ao Zendesk', 'codigo' => 7, 'demandas_id' => $demanda_inserida_id ]);    

                                   //envia email
                                   $numeroDemanda_corrente_value = $demanda_detalhes->return->numeroDemanda;
                                   Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $numeroDemanda_corrente_value ){
                                    $message->to($to_email)
                                    ->subject('Log - integrador')
                                    ->from('joao.peixoto.cd@gmail.com')  
                                    ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b> Não foi possivel conectar-se ao Zendesk para enviar esta demanda.<br> Número Demanda: '.$numeroDemanda_corrente_value.'<br><br><b>Código de erro:</b> 7'.'<br><b>Instituição:</b>'.$instituicao_corrente_value , 'text/html');
                                    });    
                                ...
                                if(empty($objetoRetornoTratado->users)){ //se não existe este email no zendesk, cria este usuario/requester
                                    ...
                                    try{ 
                                        ...
                                        LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Não foi possivel conectar-se ao zendesk para criar novo usuário-requester', 'codigo' => 8, 'demandas_id' => $demanda_inserida_id ]);    
                                        ...
                                        Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $numeroDemanda_corrente_value){
                                            ...
                                            ->subject('Log - integrador')
                                            ->from('joao.peixoto.cd@gmail.com')  
                                            ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b> Não foi possivel conectar-se ao zendesk para criar novo usuário-requester. Demanda não pode ser criada no Zendesk<br> Número Demanda: '.$numeroDemanda_corrente_value.'<br><br><b>Código de erro:</b> 8'.'<br><b>Instituição:</b>'.$instituicao_corrente_value , 'text/html');
                                        });    
                                    ...
                                    if(is_array($objetoRetornoTratado->users) && count($objetoRetornoTratado->users) > 1){
                                        ...
                                        LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Não foi possível criar demanda no zendesk, email duplicado no Zendesk', 'codigo' => 9, 'demandas_id' => $demanda_inserida_id ]);              
                                        ...
                                        Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $numeroDemanda_corrente_value ){
                                            ...
                                            ->subject('Log - integrador')
                                            ->from('joao.peixoto.cd@gmail.com')  
                                            ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b> Não foi possível criar demanda no zendesk, email duplicado no Zendesk<br> Número Demanda: '.$numeroDemanda_corrente_value.'<br><br><b>Código de erro:</b> 9'.'<br><b>Instituição:</b>'.$instituicao_corrente_value , 'text/html');
                                        });    
                                    ...
                                    if(!is_array($objetoRetornoTratado->users)){
                                        ...
                                        LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Retorno inválido do ZENDESK', 'codigo' => 10, 'demandas_id' => $demanda_inserida_id ]);    
                                        ...
                                        Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $numeroDemanda_corrente_value ){
                                            ...
                                            ->subject('Log - integrador')
                                            ->from('joao.peixoto.cd@gmail.com')  
                                            ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b>Não foi possível criar esta demanda no Zendesk. Retorno inválido do ZENDESK na criação do Requester.<br> Número Demanda: '.$numeroDemanda_corrente_value.'<br><br><b>Código de erro:</b> 10'.'<br><b>Instituição:</b>'.$instituicao_corrente_value , 'text/html');
                                        });    
                                ...
                                $body->ticket->subject = "RDR ".$demanda_detalhes->return->numeroDemanda." - Due date ".($demanda_detalhes->return->prazo?$this->formataDataBR($demanda_detalhes->return->prazo):'');    
                                ...
                                try{ 
                                    ...
                                    LogsErros::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Não foi possivel conectar-se ao Zendesk para criar o ticket', 'codigo' => 11, 'demandas_id' => $demanda_inserida_id ]);            
                                    ...
                                    Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $numeroDemanda_corrente_value){
                                        ...
                                        ->subject('Log - integrador')
                                        ->from('joao.peixoto.cd@gmail.com')  
                                        ->setBody('<b>comando:</b> consultarnovasdemandasbacen<br><br>'.'<b>Mensagem:</b> Não foi possivel conectar-se ao Zendesk para criar o ticket<br> Número Demanda: '.$numeroDemanda_corrente_value.'<br><br><b>Código de erro:</b> 11'.'<br><b>Instituição:</b>'.$instituicao_corrente_value , 'text/html');
                                    });     
                            ...
                            Demandas::where([['id', '=', $demanda_inserida_id]])->update(['zendesk_requester_id' => $requester_id, 'zendesk_ticket_id' => $createdTicketZendeskTratado->ticket->id, 'situacao_no_robo' => 'nova_enviada_ao_zendesk']);       
        ...
        if($totalDemandas->return && is_int($totalDemandas->return)){
            LogsComandos::create(['comando' => 'consultarnovasdemandasbacen', 'data' => date('Y-m-d H:i:s'), 'duracao' => 0, 'totalDemandasRetornadasRDRWebservice' => $totalDemandas->return ]);
