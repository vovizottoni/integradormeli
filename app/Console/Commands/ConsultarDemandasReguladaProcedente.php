...
class ConsultarDemandasReguladaProcedente extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'consultardemandasreguladaprocedente';
     ...
     * @var string
     */
    protected $description = 'Consulta demandas procedentes no BACEN WebService na data corrente e armazena na BASE MYSQL do Robô, depois envia para o Zendesk';
    ...
    public function handle()
    {
        ...
        try{
            ...
            $soapClient = new \SoapClient($URL_WEBSERVICE_RDR_BACEN, $optionsSoap);              
                ...
                LogsErros::create(['comando' => 'consultardemandasreguladaprocedente', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN indisponível ou credenciais desativadas/alteradas', 'codigo' => 1 ]);
                ...
                Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value){
                    $message->to($to_email)
                      ...
                      ->setBody('<b>comando:</b> consultardemandasreguladaprocedente<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN indisponível ou credenciais desativadas/alteradas'.'<br><br><b>Código de erro:</b> 1'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
                });     
        ...
        //requisita demandas reguladas procedentes
        //Algoritmo
        
        //Total de demandas "regulada procedente" na data corrente
        $parametros = [
            ...
            'situacao' => 39  //Encerrada: reclamação regulada procedente                    
        ];
        ...
        try{
            ...
            LogsErros::create(['comando' => 'consultardemandasreguladaprocedente', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN indisponível ou credenciais desativadas/alteradas ou método getTotalDemandas indisponível', 'codigo' => 2 ]);
            ...
            Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value){
                $message->to($to_email)
                  ->subject('Log - integrador')
                  ->from('joao.peixoto.cd@gmail.com')  
                  ->setBody('<b>comando:</b> consultardemandasreguladaprocedente<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN indisponível ou credenciais desativadas/alteradas ou método getTotalDemandas indisponível'.'<br><br><b>Código de erro:</b> 2'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
            });      
        ...
        if(!is_object($totalDemandas) ||  (is_object($totalDemandas) && !property_exists($totalDemandas, "return")) || (is_object($totalDemandas) && property_exists($totalDemandas, "return") && !is_int($totalDemandas->return))){
            ...
            LogsErros::create(['comando' => 'consultardemandasreguladaprocedente', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'WebService RDR BACEN retornou um valor inválido para o número de demandas reguladas procedentes que foi consultado', 'codigo' => 3 ]);  
            ...
            Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value){
                $message->to($to_email)
                  ->subject('Log - integrador')
                  ->from('joao.peixoto.cd@gmail.com')  
                  ->setBody('<b>comando:</b> consultardemandasreguladaprocedente<br><br>'.'<b>Mensagem:</b> WebService RDR BACEN retornou um valor inválido para o número de demandas reguladas procedente que foi consultado'.'<br><br><b>Código de erro:</b> 3'.'<br><b>Instituição:</b>'.$instituicao_corrente_value, 'text/html');
            });     
        ...
        if($totalDemandas->return == 0){ //Nenhuma nova demanda procedente
            ...
            LogsErros::create(['comando' => 'consultardemandasreguladaprocedente', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Ocorrência: nenhuma  demanda regulada procedente foi encontrada', 'codigo' => 4 ]);   
                    ...
                    foreach($demandas->return as $key_demanda => $demanda){                     
                        if(is_object($demanda)){
                            ...
                            if(!is_object($demanda_detalhes) || (is_object($demanda_detalhes) && !property_exists($demanda_detalhes, "return")) || !is_int($demanda_detalhes->return->idInterno) || !is_int($demanda_detalhes->return->idEncaminhamento)){
                                   ...
                                   LogsErros::create(['comando' => 'consultardemandasreguladaprocedente', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Falha na leitura de demanda do WebServive BACEN: Demanda com valores obrigatórios ausentes', 'codigo' => 5 ]);  
                            ...
                            if(empty($checka_DemandaEXISTE)){
                                ...
                                LogsErros::create(['comando' => 'consultardemandasreguladaprocedente', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Demanda foi regulada Procedente mas não existe no zendesk nem robô meli. Portanto fica presa no robô e não será enviada ao zendesk.', 'codigo' => 70, 'demandas_id' => $demanda_detalhes->return->idInterno ]);    
                                ...
                                if($checka_DemandaEXISTE->situacao_no_robo == 'regulada_procedente_nao_enviada_ao_zendesk' || $checka_DemandaEXISTE->situacao_no_robo == 'regulada_procedente_enviada_ao_zendesk'){
                            ...
                            if($situacao_bacen_id){
                                ...
                                Demandas::where([['id', '=', $checka_DemandaEXISTE->id]])->update(['situacao_bacen_id' => $situacao_bacen_id, 'situacao_no_robo' => 'regulada_procedente_nao_enviada_ao_zendesk' ]);
                            ...
                            try{ 
                                ...
                                $body->ticket->comment->body = "Demanda Regulada Procedente";
                                ...
                                $obj1->value = 'encerrada_reclamacao_regulada_procedente';
                                ...
                                LogsErros::create(['comando' => 'consultardemandasreguladaprocedente', 'data' => date('Y-m-d H:i:s'), 'descricao' => 'Não foi possivel conectar-se ao zendesk para criar o comentário do ticket (comentário: Regulada Procedente) :'.$e->getMessage() , 'codigo' => 71, 'demandas_id' => $checka_DemandaEXISTE->id ]);            
                                ...
                                Mail::send(array(), array(), function ($message) use ($to_email, $instituicao_corrente_value, $n_demanda_p_email){
                                    ...
                                    ->subject('Log - integrador')
                                    ->from('joao.peixoto.cd@gmail.com')  
                                    ->setBody('<b>comando:</b> consultardemandasreguladaprocedente<br><br>'.'<b>Mensagem:</b> Demanda foi regulada procedente mas não foi possivel conectar-se ao zendesk para criar o comentário do ticket (comentário: Regulada Procedente). Falha ao enviar ao Zendesk.'.'<br><br><b>Código de erro:</b> 71'.'<br><b>Instituição:</b>'.$instituicao_corrente_value.' <br><b>Numero Demanda:</b>'.$n_demanda_p_email, 'text/html');
                                });     
                            ...
                            Demandas::where([['id', '=', $checka_DemandaEXISTE->id]])->update(['situacao_no_robo' => 'regulada_procedente_enviada_ao_zendesk' ]);       
       ...
       //Display Script End time
        ...
        if($totalDemandas->return && is_int($totalDemandas->return)){
            LogsComandos::create(['comando' => 'consultardemandasreguladaprocedente', 'data' => date('Y-m-d H:i:s'), 'duracao' => $execution_time, 'totalDemandasRetornadasRDRWebservice' => $totalDemandas->return ]);
        }              
